...
public class RecognitionManager {
    ...
    private static int numberOfGuestureInputs, numberOfGestures, currentGesture, stopSize,gestureSize, g, constellationNumber;
    ...
    public static String filenameFinal, filenameIntermid, csvPathH,modelPathH,csvPathG,modelPathG, csvPathL, modelPathL,gesture1,gesture2,gesture3;
    ...
    private static int NUMBER_OF_STATES_F1,NUMBER_OF_STATES_F2, NUMBER_OF_STATES_F3;
    ...
    public int getNumberOfInputs() {return NUMBER_OF_INPUTS; }

    public Classifier getClassifierSVMG() {return classifierSVMG; }

    public Classifier getClassifierSVMH() {return classifierSVMH; }
    ...
    public int predict(int[] bufferSerial) throws InterruptedException {
        ...
        //Log.i(TAG, "Level & Horizontal : " + level + " " + horizontal);
        if (horizontal == 0) state = 0;

        int numFingers=0;
        if(level>NUMBER_OF_STATES_F1)numFingers=1;
        if(level>NUMBER_OF_STATES_F2)numFingers=2;
        if(level>NUMBER_OF_STATES_F3)numFingers=3;
    ...
    public void initVariables() throws IOException, JSONException, java.text.ParseException {
        ...
        */
        ...
        try {
            reader = new BufferedReader(new InputStreamReader(appContext.getAssets().open("config/mainConfig.json")));
            if (reader!= null)
        ...
        try {
            reader = new BufferedReader(new InputStreamReader(appContext.getAssets().open("config/devices/"+device_Name+"/json/variables.json")));
            if ( reader != null)
        ...
        try {
            reader = new BufferedReader(new InputStreamReader(appContext.getAssets().open(deviceConfigPath)));
            if ( reader != null)
