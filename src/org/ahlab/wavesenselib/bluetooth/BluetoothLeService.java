...
package org.ahlab.wavesenselib.bluetooth;
...
public class BluetoothLeService extends Service {
    ...
    boolean fillBufferStarted = false;
    int fillBufferIteration = 0;
    ...
    private void broadcastUpdate(final String action,
        ...
        if (characteristic.getUuid().equals(UUID.fromString(SampleGattAttributes.ZSENSE_CHARACTERISTIC))) {
            ...
            //Log.i(TAG, "Last Value: " + ((Byte)data[data.length - 1]).intValue()); 
            /*for(byte eachbyte : data)
            	Log.i(TAG,"" + ((Byte)eachbyte).intValue());*/
            	
            if(((Byte)data[data.length - 1]).intValue() == 127){
            	fillBufferStarted = true;
            	BLeData.getInstance().resetBLeDataBuffer();
            }
            if (fillBufferStarted){
            	byte[] decodedData = decodewaveSenseBleDataNew(data);
            	fillBufferIteration++;
            	//Log.i(TAG, "Iteration: " + fillBufferIteration);
            	...
            	if(fillBufferIteration > 3){
            		fillBufferStarted = false;
            		fillBufferIteration = 0;
            		//Log.i(TAG, "broadcastUpdate: " + BLeData.getInstance().getIterationNumber() + " " + Arrays.toString(BLeData.getInstance().getBLeDataBuffer()));
            		//Log.i(TAG, "Total Data Length: " + BLeData.getInstance().getBLeDataBuffer().length);
                    intent.putExtra(EXTRA_DATA, BLeData.getInstance().getBLeDataBuffer());
                    sendBroadcast(intent); 
            	}
            }
            /*for(Byte b : data){
            	Log.i(TAG, "" + b.intValue());
            }*/
            
            
            
            
            /*byte[] decodedData = decodewaveSenseBleData(data);
//            byte[] decodedData =    decodezSenseBleData(data);
            //Log.i(TAG, "" + decodedData.length);
    ...
    private byte[] decodewaveSenseBleDataNew(byte[] bytes){
    	byte[] decodedBleData = new byte[(bytes.length - 1) * 6];
    	for(int i = 0; i < bytes.length - 1; i++){
    		for(int j = 0; j < 6; j++)
    			decodedBleData[(i * 6) + j] = getBit(bytes[i], (j + 2));
    	}
    	return decodedBleData;
    }

    private byte[] decodewaveSenseBleData(byte[] bytes){
    	byte[] decodedBleData = new byte[bytes.length * 8];
    	for(int i = 0; i < bytes.length; i++){
    		for(int j = 0; j < 8; j++)
    			decodedBleData[(i * 8) + j] = getBit(bytes[i], j);
    	}
    	return decodedBleData;
    }
    
    public byte getBit(byte singlebyte, int position)
    {
       return (byte)((singlebyte >> position) & 1);
    }

    private byte[] decodezSenseBleData(byte[] p_encoded_buffer){
        int MAX_ZS_LEN = 22;
